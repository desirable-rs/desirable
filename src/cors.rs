...
#[derive(Clone, Debug)]
pub struct Cors {
  allow_origin: header::HeaderValue,
  allow_methods: header::HeaderValue,
  allow_headers: header::HeaderValue,
  allow_credentials: bool,
  max_age: header::HeaderValue,
  expose_headers: Option<header::HeaderValue>,
}

/// Parses a configured CORS value into a header value once, at construction
/// time, so the request path never validates or allocates.
fn header_value(value: &str) -> header::HeaderValue {
  value.parse().expect("invalid CORS header value")
}
...
impl Cors {
  ...
  pub fn new() -> Self {
    Self {
      ...
      allow_methods: header_value("GET, POST, PUT, DELETE, PATCH, OPTIONS, HEAD"),
      allow_headers: header_value("Content-Type, Authorization"),
      allow_credentials: false,
      max_age: header_value("86400"),
      expose_headers: None,
    }
  }
  ...
  pub fn allow_origin(mut self, origin: &str) -> Self {
    self.allow_origin = header_value(origin);
    self
  }
  ...
  pub fn allow_methods(mut self, methods: &str) -> Self {
    self.allow_methods = header_value(methods);
    self
  }
  ...
  pub fn allow_headers(mut self, headers: &str) -> Self {
    self.allow_headers = header_value(headers);
    self
  }
  ...
  pub fn max_age(mut self, seconds: u32) -> Self {
    self.max_age = header_value(&seconds.to_string());
    self
  }
  ...
  pub fn expose_headers(mut self, headers: &str) -> Self {
    self.expose_headers = Some(header_value(headers));
    self
  }
...
mod tests {
  ...
  fn test_cors_defaults() {
    let cors = Cors::new();
    assert_eq!(cors.allow_origin, header_value("*"));
    assert_eq!(
      cors.allow_methods,
      header_value("GET, POST, PUT, DELETE, PATCH, OPTIONS, HEAD")
    );
    assert!(!cors.allow_credentials);
    assert_eq!(cors.max_age, header_value("86400"));
  }
  ...
  fn test_cors_builder() {
    ...
    assert_eq!(cors.allow_origin, header_value("https://example.com"));
    assert_eq!(cors.allow_methods, header_value("GET, POST"));
    assert_eq!(cors.allow_headers, header_value("X-Custom"));
    assert!(cors.allow_credentials);
    assert_eq!(cors.max_age, header_value("3600"));
    assert_eq!(cors.expose_headers, Some(header_value("X-Response-Id")));
  }
